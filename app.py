...
st.set_page_config(page_title="Traductor Ing. Química", page_icon="🧪", layout="wide")
...
chunk_size = st.sidebar.slider("Páginas por lote", 5, 50, DEFAULT_CHUNK_SIZE)
st.sidebar.info("Para PDFs de +1000 hojas, usa lotes pequeños (15-20) para no saturar la memoria.")

uploaded_file = st.file_uploader("📂 Sube el archivo PDF en Inglés", type=["pdf"])
...
if uploaded_file is not None:
    ...
    if st.button("🚀 Iniciar Traducción", type="primary"):
        if not api_key:
            st.error("⚠️ Necesitas ingresar tu API Key de OpenAI en la barra lateral.")
        else:
            ...
            try:
                engine = PDFTranslatorEngine(api_key=api_key)
